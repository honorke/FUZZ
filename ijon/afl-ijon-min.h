...
extern map_int_t * extras_hitmap;
...
static void destroy_rule(ijon_rule * rule){
	if(rule->s_chunk){
		ck_free(rule->s_chunk);
	}
	if(rule->s_chunk){
		ck_free(rule->t_chunk);
	}
	rule->next=NULL;
	ck_free(rule);
}
...
//add by dongke
static void head_insert_rule_to_ijon_rules(ijon_rule * rule_head){
	if (!rule_head)
	{
		return;
	}
	rule_head->next = ijon_rules;
	ijon_rule = rule_head;
	return;
}
